...
bool BLIND = true;
bool SIDEBAND = false;
bool DROPLAURENT = false;
bool runFtestCheckWithToys=false;
int mzg_low =115; // JB
int mzg_high =170; // JB
int nBinsForMass = 4*(mzg_high-mzg_low);

RooRealVar *intLumi_ = new RooRealVar("IntLumi","hacked int lumi", 35.9);
...
double getProbabilityFtest(double chi2, int ndof,RooAbsPdf *pdfNull, RooAbsPdf *pdfTest, RooRealVar *mass, RooDataSet *data, std::string name){
  ...
  if (SIDEBAND) {
      fitNullData = pdfNull->fitTo(*data, RooFit::Save(1),Range("left,right"),RooFit::Strategy(1),RooFit::Minimizer("Minuit2","minimize"),RooFit::SumW2Error(kTRUE),RooFit::PrintLevel(-1)); //FIXME
      fitTestData = pdfTest->fitTo(*data, RooFit::Save(1),Range("left,right"),RooFit::Strategy(1),RooFit::Minimizer("Minuit2","minimize"),RooFit::SumW2Error(kTRUE),RooFit::PrintLevel(-1)); //FIXME
  }
  else {
      fitNullData = pdfNull->fitTo(*data, RooFit::Save(1),RooFit::Strategy(1),RooFit::Minimizer("Minuit2","minimize"),RooFit::SumW2Error(kTRUE),RooFit::PrintLevel(-1)); //FIXME
      fitTestData = pdfTest->fitTo(*data, RooFit::Save(1),RooFit::Strategy(1),RooFit::Minimizer("Minuit2","minimize"),RooFit::SumW2Error(kTRUE),RooFit::PrintLevel(-1)); //FIXME
  }
  ...
  for (int itoy = 0 ; itoy < ntoys ; itoy++){
	...
	while (stat_n!=0){
      ...
      if (SIDEBAND) {
          fitNull = pdfNull->fitTo(*binnedtoy,Range("left,right"),RooFit::Save(1),RooFit::Strategy(1),RooFit::SumW2Error(kTRUE),RooFit::Minimizer("Minuit2","minimize"),RooFit::Minos(0),RooFit::Hesse(0),RooFit::PrintLevel(-1));
      }
      else {
          fitNull = pdfNull->fitTo(*binnedtoy,RooFit::Save(1),RooFit::Strategy(1),RooFit::SumW2Error(kTRUE),RooFit::Minimizer("Minuit2","minimize"),RooFit::Minos(0),RooFit::Hesse(0),RooFit::PrintLevel(-1));
      }
		//,RooFit::Optimize(0));
	...
	while (stat_t!=0){
      ...
      RooFitResult *fitTest;
      if (SIDEBAND) {
          fitTest = pdfTest->fitTo(*binnedtoy,Range("left,right"),RooFit::Save(1),RooFit::Strategy(1),RooFit::SumW2Error(kTRUE),RooFit::Minimizer("Minuit2","minimize"),RooFit::Minos(0),RooFit::Hesse(0),RooFit::PrintLevel(-1));
      }
      else {
          fitTest = pdfTest->fitTo(*binnedtoy,RooFit::Save(1),RooFit::Strategy(1),RooFit::SumW2Error(kTRUE),RooFit::Minimizer("Minuit2","minimize"),RooFit::Minos(0),RooFit::Hesse(0),RooFit::PrintLevel(-1));
      }
...
double getGoodnessOfFit(RooRealVar *mass, RooAbsPdf *mpdf, RooDataSet *data, std::string name){
  ...
  //RooAddPdf *pdf = new RooAddPdf("ext","ext",RooArgList(*mpdf),RooArgList(norm));
  ...
  if ((double)data->sumEntries()/nBinsForMass < 5 ){
    ...
    //  std::cout << "[INFO] " <<Form("\t.. %.1f %% complete\r",100*float(itoy)/ntoys) << std::flush;
      ...
      if (SIDEBAND) {
          pdf->fitTo(*binnedtoy,Range("left,right"),RooFit::Minimizer("Minuit2","minimize"),RooFit::Minos(0),RooFit::Hesse(0),RooFit::PrintLevel(-1),RooFit::Strategy(0),RooFit::SumW2Error(kTRUE)); //FIXME
      }
      else {
...
void plot(RooRealVar *mass, RooAbsPdf *pdf, RooDataSet *data, string name,vector<string> flashggCats_, int status, double *prob){
  ...
  *prob = getGoodnessOfFit(mass,pdf,data,name);
  ...
  mass->setRange("unblindReg_1",mzg_low,120); // JB
  mass->setRange("unblindReg_2",130,mzg_high); // JB
  if (BLIND) {
  ...
  TCanvas *canv = new TCanvas();
  if (SIDEBAND) {
      pdf->plotOn(plot, RooFit::Range(115,170), RooFit::NormRange("left,right"));//,RooFit::NormRange("fitdata_1,fitdata_2"));
...
void plot(RooRealVar *mass, RooMultiPdf *pdfs, RooCategory *catIndex, RooDataSet *data, string name, vector<string> flashggCats_, int cat, int bestFitPdf=-1){
  ...
  for (int icat=0;icat<catIndex->numTypes();icat++){
    ...
    if (SIDEBAND) {
        pdfs->getCurrentPdf()->fitTo(*data,Range("left,right"),RooFit::Minos(0),RooFit::Minimizer("Minuit2","minimize"),RooFit::SumW2Error(kTRUE));	 //FIXME
...
void plot(RooRealVar *mass, map<string,RooAbsPdf*> pdfs, RooDataSet *data, string name, vector<string> flashggCats_, int cat, int bestFitPdf=-1){
  ...
  for (map<string,RooAbsPdf*>::iterator it=pdfs.begin(); it!=pdfs.end(); it++){
    ...
    it->second->plotOn(plot,LineColor(col),LineStyle(style), RooFit::NormRange("left,right"));
    TObject *pdfLeg = plot->getObject(int(plot->numItems()-1));
    std::string ext = "";
...
vector<string> flashggCats_;
 bool isData_ =0;
  ...
  desc.add_options()
    ...
    ("channel", po::value<string>(&channel),                                                     "analysis channel (eeg, mmg)")
    ("era", po::value<string>(&era),                                                     "analysis era (2016, 2017, 2018)") //JB
    ("rangeMin", po::value<float>(&rangeMin)->default_value(115.),                                                     "lower bound of fit range") //JB
    ("rangeMax", po::value<float>(&rangeMax)->default_value(170.),                                                     "upper bound of fit range") //JB
    ("sidebandOnly",                                             "only fit sidebands")
    ("dropLaurent",                                             "don't use the Laurent series")
  ...
  if (vm.count("sidebandOnly")) SIDEBAND=true;
  if (vm.count("dropLaurent")) DROPLAURENT=true;
  ...
  RooWorkspace *inWS;
	if(isFlashgg_){
		if (isData_){
			...
			inWS = (RooWorkspace*)inFile->Get("cms_hzg_workspace"); // JB
		} else {
			inWS = (RooWorkspace*)inFile->Get("cms_hzg_workspace");
		}
	} else {
    ...
    mass->setRange(rangeMin, rangeMax); // JB for fit study (FULL RANGE)
    //mass->setRange("left", rangeMin, 120.);
    //mass->setRange("right", 130., rangeMax);
    mass->setRange("left", rangeMin, 120.);
    mass->setRange("right", 130., rangeMax);
    ...
    nBinsForMass = 4*(rangeMax-rangeMin);
    mzg_low = rangeMin;
    mzg_high = rangeMax;

    char cuts[50];
    sprintf(cuts, "CMS_hzg_mass < %f", rangeMax);

    //char sidebandCuts[50] = "";
    //if (sidebandOnly) sprintf(sidebandCuts, "(CMS_hzg_mass<120.)||(CMS_hzg_mass>130.)");

    //std::cout << "sidebandOnly = " << sidebandOnly << std::endl;
    //std::cout << "sidebandCuts = " << sidebandCuts << std::endl;

	for (int cat=startingCategory; cat<ncats; cat++){
    
        if (flashggCats_[cat] == "6789") {
            pdfsModel.setOddLaurentBounds(0.0000000001, 0.9999999999);
        }
        else {
            pdfsModel.setOddLaurentBounds(-2., 5.);
        }
		...
		if (isData_) {
    dataFull = (RooDataSet*)(inWS->data(Form("Data_13TeV_%s",catname.c_str()))->reduce(Cut(cuts)));
    ...
    {dataFull = (RooDataSet*)inWS->data(Form("data_mass_%s",catname.c_str()));
		...
		if ( isFlashgg_){
			thisdataBinned_name =Form("roohist_data_mass_%s_cat%s_%s", channel.c_str(), flashggCats_[cat].c_str(), era.c_str());
			//	RooDataHist thisdataBinned(Form("roohist_data_mass_cat%d",cat),"data",*mass,*dataFull);
		...
		for (vector<string>::iterator funcType=functionClasses.begin(); 
			...
			while (prob<0.05 && order < 7){ //FIXME
				...
				if (!bkgPdf){
					...
					//runFit(bkgPdf,(RooDataSet*)data->reduce(Cut(sidebandCuts)),&thisNll,&fitStatus,/*max iterations*/3);//bkgPdf->fitTo(*data,Save(true),RooFit::Minimizer("Minuit2","minimize"));
					...
					if (prev_pdf!=NULL){
						...
						//prob = getProbabilityFtest(chi2,order-prev_order,prev_pdf,bkgPdf,mass,(RooDataSet*)data->reduce(Cut(sidebandCuts))
						//		,Form("%s/Ftest_from_%s%d_cat%d.pdf",outDir.c_str(),funcType->c_str(),order,cat));
			...
			if (saveMultiPdf){
				...
				while (prob<upperEnvThreshold){
					RooAbsPdf *bkgPdf = getPdf(pdfsModel,*funcType,order,Form("env_pdf_%d_%s_%s_%s",cat,ext.c_str(),channel.c_str(), era.c_str()));
					if (!bkgPdf ){
						// assume this order is not allowed
						...
						double gofProb =0; 
                        //RooAbsPdf *bkgPdfForGOF(bkgPdf);
						plot(mass,bkgPdf,data,Form("%s/%s%d_cat%d.pdf",outDir.c_str(),funcType->c_str(),order,cat),flashggCats_,fitStatus,&gofProb);
						//plot(mass,bkgPdfForGOF,data,Form("%s/%s%d_cat%d.pdf",outDir.c_str(),funcType->c_str(),order,cat),flashggCats_,fitStatus,&gofProb);
						...
						if ((prob < upperEnvThreshold) ) { // Looser requirements for the envelope
							...
							if (gofProb > 0.01 || order == truthOrder ) {  // Good looking fit or one of our regular truth functions
								...
								//allPdfs.insert(pair<string,RooAbsPdf*>(Form("%s%d",funcType->c_str(),order),bkgPdfForRunFit));
		...
		if (saveMultiPdf){
			...
			string catname;
			if (isFlashgg_){
				catindexname = Form("pdfindex_%s_cat%s_%s_%s",channel.c_str(),flashggCats_[cat].c_str(),era.c_str(),ext.c_str());
				catname = Form("%s",flashggCats_[cat].c_str());
			} else {
			...
			RooMultiPdf *pdf = new RooMultiPdf(Form("CMS_hzg_%s_cat%s_%s_%s_bkgshape",channel.c_str(),catname.c_str(),era.c_str(),ext.c_str()),"all pdfs",catIndex,storedPdfs);
			//RooRealVar nBackground(Form("CMS_hgg_%s_%s_bkgshape_norm",catname.c_str(),ext.c_str()),"nbkg",data->sumEntries(),0,10E8);
			RooRealVar nBackground(Form("CMS_hzg_%s_cat%s_%s_%s_bkgshape_norm",channel.c_str(),catname.c_str(),era.c_str(),ext.c_str()),"nbkg",data->sumEntries(),0,3*data->sumEntries());
			...
			mass->setBins(nBinsForMass);
			RooDataHist dataBinned(Form("roohist_data_mass_%s_cat%s_%s", channel.c_str(), catname.c_str(), era.c_str()),"data",*mass,*dataFull);

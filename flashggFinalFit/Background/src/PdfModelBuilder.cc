...
void PdfModelBuilder::setOddLaurentBounds(double lower, double upper){
    oddLaurentLower = lower;
    oddLaurentUpper = upper;
}
...
RooAbsPdf* PdfModelBuilder::getPowerLaw(string prefix, int order){
  ...
  for (int i=0; i<order; i++){
    double start=-2.;
    double low=-20.;//10.-default
    double high=10.;//0.-default
    if (order>0){
...
RooAbsPdf* PdfModelBuilder::getLaurentSeries(string prefix, int order){
  ...
  for (int i=1; i<=nlower; i++){
    ...
    //params.insert(pair<string,RooRealVar*>(name, new RooRealVar(name.c_str(),name.c_str(),0.25/order,0.000001,0.999999))); // old
    params.insert(pair<string,RooRealVar*>(name, new RooRealVar(name.c_str(),name.c_str(),0.25/order,0.000000001,0.999999999999)));
								//0.999999)));
  ...
  for (int i=1; i<=nhigher; i++){
    ...
    //params.insert(pair<string,RooRealVar*>(name, new RooRealVar(name.c_str(),name.c_str(),0.25/order,0.0000000001,0.9999999999)));
    params.insert(pair<string,RooRealVar*>(name, new RooRealVar(name.c_str(),name.c_str(),0.25/order,oddLaurentLower,oddLaurentUpper)));
    //params.insert(pair<string,RooRealVar*>(name, new RooRealVar(name.c_str(),name.c_str(),0.25/order,0.000001,0.999999))); // old
    //params.insert(pair<string,RooRealVar*>(name, new RooRealVar(name.c_str(),name.c_str(),0.25/order,-2,5.)));
    plist->add(*params[name]);

...
class HGCalTriggerChains:
    ...
    def register_calo(self, name, generator): # for truth-matched trigger cells
        self.calo[name] = generator
    ...
    def register_truth_chain(self, vfe, calo, selector='', ntuple=''):
        if not vfe in self.vfe: 
            raise KeyError('{} not registered as VFE producer'.format(vfe))
        if not calo in self.calo: 
            raise KeyError('{} not registered as calo producer'.format(calo))
        self.truth_chain.append( (vfe, calo, selector, ntuple) )

    def register_truth_be_chain(self, vfe, calo, backend1, backend2, selector='', ntuple=''): 
        if not vfe in self.vfe: 
            raise KeyError('{} not registered as VFE producer'.format(vfe))
        if not calo in self.calo: 
            raise KeyError('{} not registered as calo producer'.format(calo))
        ...
        if ntuple!='' and not ntuple in self.ntuple: 
            raise KeyError('{} not registered as ntuplizer'.format(ntuple))
        self.truth_be_chain.append( (vfe, calo, backend1, backend2, selector, ntuple) )
    ...
    def create_sequences(self, process):
        ...
        calo_sequence = cms.Sequence(tmp)
        ...
        for vfe,calo,selector,ntuple in self.truth_chain:
            calo_name = '{0}{1}'.format(vfe, calo)
            ntuple_name = '{0}{1}{2}'.format(vfe, calo, ntuple)
            if selector!='' and not hasattr(process, selector_name):
                setattr(process, selector_name, self.selector[selector](process, calo_name))
                selector_sequence *= getattr(process, selector_name)
            if selector=='':
                ntuple_inputs = [calo_name,
                            '',
                            calo_name
                            ]
            else:
            ...
            if not hasattr(process, vfe):
                setattr(process, vfe, self.vfe[vfe](process))
                vfe_sequence *= getattr(process, vfe)
            if not hasattr(process, calo_name):
                setattr(process, calo_name, self.calo[calo](process, vfe))
                calo_sequence *= getattr(process, calo_name)
            if selector!='' and not hasattr(process, selector_name):
                setattr(process, selector_name, self.selector[selector](process, backend2_name))
                selector_sequence *= getattr(process, selector_name)
            if ntuple!='' and not hasattr(process, ntuple_name):
                setattr(process, ntuple_name, self.ntuple[ntuple](process, ntuple_inputs))
                ntuple_sequence *= getattr(process, ntuple_name)
        for vfe,calo,backend1,backend2,selector,ntuple in self.truth_be_chain:
            calo_name = '{0}{1}'.format(vfe, calo)
            backend1_name = '{0}{1}{2}'.format(vfe, calo, backend1)
            backend2_name = '{0}{1}{2}{3}'.format(vfe, calo, backend1, backend2)
            selector_name = '{0}{1}{2}{3}{4}'.format(vfe, calo, backend1, backend2, selector)
            ntuple_name = '{0}{1}{2}{3}{4}{5}'.format(vfe, calo, backend1, backend2, selector, ntuple)
            if selector=='':
                ntuple_inputs = [
                        calo_name,
                        backend1_name+':HGCalBackendLayer1Processor2DClustering',
            ...
            if not hasattr(process, calo_name):
                ...
                calo_sequence *= getattr(process, calo_name)
            if not hasattr(process, backend1_name):
                setattr(process, backend1_name, self.backend1[backend1](process, calo_name))
                backend1_sequence *= getattr(process, backend1_name)
            if not hasattr(process, backend2_name):
                setattr(process, backend2_name, self.backend2[backend2](process, backend1_name))
                backend2_sequence *= getattr(process, backend2_name)
        ...
        process.globalReplace('caloTruthCells', calo_sequence)
